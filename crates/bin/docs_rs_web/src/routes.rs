...
#[instrument(skip_all)]
fn get_internal<H, T, S>(metrics: &Arc<WebMetrics>, handler: H) -> MethodRouter<S, Infallible>
where
    H: AxumHandler<T, S>,
    T: 'static,
    S: Clone + Send + Sync + 'static,
{
    get(handler).route_layer(middleware::from_fn_with_state(
        metrics.clone(),
        |State(metrics), request, next| async {
            request_recorder(metrics, request, next, None).await
        },
    ))
}

#[instrument(skip_all)]
fn post_internal<H, T, S>(metrics: &Arc<WebMetrics>, handler: H) -> MethodRouter<S, Infallible>
where
    H: AxumHandler<T, S>,
    T: 'static,
    S: Clone + Send + Sync + 'static,
...
#[instrument(skip_all)]
fn get_rustdoc<H, T, S>(metrics: &Arc<WebMetrics>, handler: H) -> MethodRouter<S, Infallible>
where
    H: AxumHandler<T, S>,
    T: 'static,
    S: Clone + Send + Sync + 'static,
{
    get(handler)
        .route_layer(middleware::from_fn_with_state(
            metrics.clone(),
            |State(metrics), request, next| async {
                request_recorder(metrics, request, next, Some("rustdoc page")).await
            },
        ))
...
pub(crate) fn build_axum_routes(metrics: &Arc<WebMetrics>) -> Result<AxumRouter<AppState>> {
    ...
    Ok(AxumRouter::new()
        ...
        .route(
            ...
            get_static(metrics, || async {
                cached_permanent_redirect("/-/static/robots.txt")
            }),
        )
        .route(
            "/favicon.ico",
        ...
        .nest_service(
            "/-/static",
            build_static_router(static_root_dir()?, metrics),
        )
        .route(
            "/opensearch.xml",
            get_static(metrics, || async {
                cached_permanent_redirect("/-/static/opensearch.xml")
            }),
        )
        .route_with_tsr(
            "/sitemap.xml",
            get_internal(metrics, sitemap::sitemapindex_handler),
        )
        .route_with_tsr(
            "/-/sitemap/recent/{date}/sitemap.xml",
            get_internal(metrics, sitemap::recent_sitemap_handler),
        )
        .route_with_tsr(
            "/-/sitemap/{letter}/sitemap.xml",
            get_internal(metrics, sitemap::sitemap_handler),
        )
        .route_with_tsr("/-/status/", get_internal(metrics, status::status_handler))
        .route_with_tsr(
            "/about/builds",
            get_internal(metrics, about::about_builds_handler),
        )
        .route_with_tsr("/about", get_internal(metrics, about::about_handler))
        .route_with_tsr(
            "/about/{subpage}",
            get_internal(metrics, about::about_handler),
        )
        .route("/", get_internal(metrics, releases::home_page))
        .route_with_tsr(
            "/releases",
            get_internal(metrics, releases::recent_releases_handler),
        )
        .route_with_tsr(
        ...
        .route_with_tsr(
            "/releases/stars",
            get_internal(metrics, releases::releases_by_stars_handler),
        )
        .route_with_tsr(
            "/releases/stars/{page}",
            get_internal(metrics, releases::releases_by_stars_handler),
        ...
        .route_with_tsr(
            ...
            get_internal(metrics, releases::releases_recent_failures_handler),
        )
        .route_with_tsr(
        ...
        .route_with_tsr(
            "/releases/failures",
            get_internal(metrics, releases::releases_failures_by_stars_handler),
        )
        .route_with_tsr(
        ...
        .route(
            "/crate/{name}",
            get_internal(metrics, crate_details::crate_details_handler),
        )
        .route(
            "/crate/{name}/",
            get_internal(metrics, crate_details::crate_details_handler),
        ...
        .route_with_tsr(
            "/releases/feed",
            get_internal(metrics, releases::releases_feed_handler),
        )
        .route_with_tsr(
            "/releases/{owner}",
            get_internal(metrics, releases::owner_handler),
        )
        .route_with_tsr(
            "/releases/{owner}/{page}",
            get_internal(metrics, releases::owner_handler),
        ...
        .route_with_tsr(
            "/releases/activity",
            get_internal(metrics, releases::activity_handler),
        )
        .route_with_tsr(
            "/releases/search",
            get_internal(metrics, releases::search_handler),
        )
        .route_with_tsr(
            "/releases/queue",
            get_internal(metrics, releases::build_queue_handler),
        )
        .route_with_tsr(
            "/crate/{name}/{version}/builds",
            get_internal(metrics, builds::build_list_handler),
        )
        .route(
            "/crate/{name}/{version}/rebuild",
            post_internal(metrics, builds::build_trigger_rebuild_handler),
        )
        .route(
            "/crate/{name}/{version}/status.json",
            get_internal(metrics, build_status::status_handler),
        )
        .route_with_tsr(
            "/crate/{name}/{version}/builds/{id}",
            get_internal(metrics, build_details::build_details_handler),
        )
        .route_with_tsr(
        ...
        .route_with_tsr(
            ...
            get_internal(metrics, features::build_features_handler),
        )
        .route_with_tsr(
            "/crate/{name}/{version}/source/",
            get_internal(metrics, source::source_browser_handler),
        )
        .route(
        ...
        .route(
            ...
            get_internal(metrics, crate_details::get_all_platforms),
        )
        .route(
        ...
        .route(
            ...
            get_internal(metrics, crate_details::get_all_platforms_root),
        )
        .route(
        ...
        .route_with_tsr(
            ...
            get_internal(metrics, rustdoc::download_handler),
        )
        .route_with_tsr(
            "/crate/{name}/{version}/json.gz",
            get_internal(metrics, rustdoc::json_download_handler),
        )
        .route_with_tsr(
        ...
        .route(
            ...
            get_internal(metrics, rustdoc::target_redirect_handler),
        )
        .route_with_tsr(
        ...
        .route(
            "/{name}/badge.svg",
            get_internal(metrics, rustdoc::badge_handler),
        )
        .route(
            "/{name}",
            get_rustdoc(metrics, rustdoc::rustdoc_redirector_handler),
        )
        .route(
            "/{name}/",
            get_rustdoc(metrics, rustdoc::rustdoc_redirector_handler),
        ...
        .route(
            ...
            get_rustdoc(metrics, rustdoc::rustdoc_html_server_handler),
        )
        .route(
