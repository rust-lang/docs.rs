...
impl RustwideBuilder {
    ...
    fn rustc_version(&self) -> Result<String> {
        self.environment.rustc_version()
    }
...
mod tests {
    ...
    use docs_rs_rustdoc_json::read_format_version_from_rustdoc_json;
    use docs_rs_types::{
    ...
    use std::{collections::BTreeMap, fs, io, iter, path::PathBuf};
    ...
    fn test_cross_compile_non_host_default() -> Result<()> {
        ...
        let mut builder = env.build_builder()?;
        builder.update_toolchain()?;
        if builder.environment.toolchain().as_ci().is_some() {
            return Ok(());
        }
        assert!(
            builder
