...
mod tests {
    ...
    fn test_failed_build_with_existing_successful_release() -> Result<()> {
        ...
        let previous_targets = block_on_async_with_conn!(env, |mut conn| async {
            Ok(
                sqlx::query_scalar::<_, Option<sqlx::types::Json<Vec<String>>>>(
        ...
        let row = publication_tests::fetch_build_result(&env, &crate_)?;
        assert_eq!(row.status, BuildStatus::Failure);
        assert_eq!(row.doc_targets, previous_targets);
        assert!(!publication_tests::fetch_build_logs(&env, row.id)?.is_empty());
        assert!(storage.exists_in_archive(
