...
impl AsyncBuildQueue {
    ...
    pub async fn has_build_queued(&self, name: &str, version: &Version) -> Result<bool> {
        let mut conn = self.db.get_async().await?;
        Ok(sqlx::query_scalar!(
            "SELECT id
             FROM queue
    ...
    pub async fn remove_crate_from_queue(&self, name: &str) -> Result<()> {
        let mut conn = self.db.get_async().await?;
        sqlx::query!(
            "DELETE
             FROM queue
    ...
    pub async fn remove_version_from_queue(&self, name: &str, version: &Version) -> Result<()> {
        let mut conn = self.db.get_async().await?;
        sqlx::query!(
            "DELETE
             FROM queue
