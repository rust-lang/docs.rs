...
impl AsyncBuildQueue {
    ...
    /// Reevaluate priorities in build queue.
    ///
    /// Applies eventually updated priority configuration to already queued releases.
    /// Non-default priorities are deliberately left unchanged.
    pub async fn reevaluate_priorities(&self) -> Result<()> {
...
#[cfg(test)]
mod tests {
    use crate::testing::test_env::TestEnv;
    ...
    use docs_rs_test_fakes::FakeGithubStats;
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_reevaluate_priorities_deprioritizes_queued_workspace() -> Result<()> {
        ...
        for name in [FOO, BAR, BAZ] {
            queue.add_crate(&name, &V1, crate::PRIORITY_DEFAULT).await?;
        }

        rewrite_repository_stats(&mut conn).await?;
        queue.priorities_cache.reload().await?;
        queue.reevaluate_priorities().await?;
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_reevaluate_priorities_applies_repository_override() -> Result<()> {
        let env = TestEnv::new().await?;
        let queue = env.queue();
        let mut conn = env.db.async_conn().await?;
        ...
        queue.add_crate(&FOO, &V1, crate::PRIORITY_DEFAULT).await?;
        queue
