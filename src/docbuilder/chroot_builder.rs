...
use crate::db::{
    add_build_into_database, add_package_into_database, add_path_into_database, connect_db,
};
use crate::error::Result;
use crate::utils::{
    command_result, copy_doc_dir, get_package, parse_rustc_version, source_path, update_sources,
};
...
impl DocBuilder {
    ...
    pub fn build_world(&mut self) -> Result<()> {
        ...
        crates(
            self.options.crates_io_index_path.clone(),
            |name, version| {
    ...
    pub fn build_package(&mut self, name: &str, version: &str) -> Result<bool> {
        ...
        let release_id =
            add_package_into_database(&conn, &pkg, &res, Some(file_list), successfully_targets)?;
    ...
    fn copy_documentation(
        ...
        let mut destination = PathBuf::from(&self.options.destination).join(format!(
            "{}/{}",
            package.manifest().name(),
            package.manifest().version()
        ));
    ...
    fn clean(&self, package: &Package) -> Result<()> {
        ...
        let documentation_path =
            PathBuf::from(&self.options.destination).join(package.manifest().name().as_str());
    ...
    pub fn get_versions(&self) -> (String, String) {
        ...
        (
            String::from(
                self.chroot_command("rustc --version")
                    .expect("Failed to get rustc version")
                    .trim(),
            ),
            String::from(
                self.chroot_command("cratesfyi --version")
    ...
    fn add_documentation_into_database(
        &self,
        conn: &Connection,
        package: &Package,
    ) -> Result<Json> {
    ...
    pub fn add_essential_files(&self) -> Result<()> {
        ...
        if !res.build_success {
            return Err(format_err!(
                "Failed to build empty crate for: {}",
                res.rustc_version
            ));
        }
        ...
        for file in files.0.iter() {
            ...
            copy(&source_path, &destination_path).chain_err(|| {
                format!(
                    "couldn't copy '{}' to '{}'",
                    source_path.display(),
        ...
        .or_else(|_| {
            conn.query(
                "UPDATE config SET value = $1 WHERE name = 'rustc_version'",
                &[&vers.to_json()],
            )
        })?;
