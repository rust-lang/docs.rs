...
pub(crate) async fn sitemapindex_handler() -> impl IntoResponse {
    ...
    SitemapIndexXml { sitemaps, csp_nonce: String::new() }
}
...
pub(crate) async fn sitemap_handler(
    ...
    Ok(SitemapXml { releases, csp_nonce: String::new() })
}
    ...
    ($ty:ident, $template:literal) => {
        ...
        impl $ty {
            fn into_response(&self) -> Response {
                ...
                //             http::HeaderValue::from_static(<Self as Template>::MIME_TYPE),
                //         )];
...
pub(crate) async fn about_handler(subpage: Option<Path<String>>) -> AxumResult<impl IntoResponse> {
    ...
    let response = match &subpage[..] {
        "about" | "index" => AboutPage { active_tab: "index" }.into_response(),
        "badges" => AboutPageBadges { active_tab: "badges" }.into_response(),
        "metadata" => AboutPageMetadata { active_tab: "metadata" }.into_response(),
        "redirections" => AboutPageRedirection { active_tab: "redirections" }.into_response(),
        "download" => AboutPageDownload { active_tab: "download" }.into_response(),
        _ => {
