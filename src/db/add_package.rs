...
pub fn add_package_into_database(conn: &Connection,
    ...
    let crate_id = r#try!(initialize_package_in_database(&conn, &pkg));
    ...
    let (release_time, yanked, downloads) = r#try!(get_release_time_yanked_downloads(&pkg));
    ...
    let release_id: i32 = {
        let rows = r#try!(conn.query("SELECT id FROM releases WHERE crate_id = $1 AND version = $2",
                                   &[&crate_id, &format!("{}", pkg.manifest().version())]));

        if rows.len() == 0 {
            let rows = r#try!(conn.query("INSERT INTO releases (
            ...
            r#try!(conn.query("UPDATE releases
                             SET release_time = $3,
                                 dependencies = $4,
                                 target_name = $5,
                                 yanked = $6,
    ...
    r#try!(add_keywords_into_database(&conn, &pkg, &release_id));
    r#try!(add_authors_into_database(&conn, &pkg, &release_id));
    r#try!(add_owners_into_database(&conn, &pkg, &crate_id));


    // Update versions
    {
        let mut versions: Json = r#try!(conn.query("SELECT versions FROM crates WHERE id = $1",
                                                 &[&crate_id]))
            .get(0)
            .get(0);
...
pub fn add_build_into_database(conn: &Connection,
    ...
    let rows = r#try!(conn.query("INSERT INTO builds (rid, rustc_version,
                                                    cratesfyi_version,
                                                    build_status, output)
                                VALUES ($1, $2, $3, $4, $5)
                                RETURNING id",
...
fn initialize_package_in_database(conn: &Connection, pkg: &Package) -> Result<i32> {
    let mut rows = r#try!(conn.query("SELECT id FROM crates WHERE name = $1",
                                   &[&pkg.manifest().name().as_str()]));
    // insert crate into database if it is not exists
    if rows.len() == 0 {
        rows = r#try!(conn.query("INSERT INTO crates (name) VALUES ($1) RETURNING id",
                               &[&pkg.manifest().name().as_str()]));
    }
    Ok(rows.get(0).get(0))
}
...
fn get_readme(pkg: &Package) -> Result<Option<String>> {
    let readme_path = PathBuf::from(r#try!(source_path(&pkg).ok_or_else(|| err_msg("File not found"))))
    ...
    let mut reader = r#try!(fs::File::open(readme_path).map(|f| BufReader::new(f)));
    let mut readme = String::new();
    r#try!(reader.read_to_string(&mut readme));
...
fn get_rustdoc(pkg: &Package) -> Result<Option<String>> {
    if let Some(src_path) = pkg.manifest().targets()[0].src_path().path() {
        if src_path.is_absolute() {
            ...
            let mut path = PathBuf::from(r#try!(source_path(&pkg).ok_or_else(|| err_msg("File not found"))));
            path.push(src_path);
...
fn read_rust_doc(file_path: &Path) -> Result<Option<String>> {
    let reader = r#try!(fs::File::open(file_path).map(|f| BufReader::new(f)));
...
fn get_release_time_yanked_downloads
    ...
    let mut res = r#try!(client.get(&url[..])
        .header(ACCEPT, "application/json")
        .send());
    let mut body = String::new();
    res.read_to_string(&mut body).unwrap();
    let json = Json::from_str(&body[..]).unwrap();
    let versions = r#try!(json.as_object()
    ...
    for version in versions {
        let version = r#try!(version.as_object().ok_or_else(|| err_msg("Not a JSON object")));
        let version_num = r#try!(version.get("num")
            .and_then(|v| v.as_string())
            .ok_or_else(|| err_msg("Not a JSON object")));

        if &semver::Version::parse(version_num).unwrap() == pkg.manifest().version() {
            let release_time_raw = r#try!(version.get("created_at")
            ...
            yanked = Some(r#try!(version.get("yanked")
                .and_then(|c| c.as_boolean())
                .ok_or_else(|| err_msg("Not a JSON object"))));

            downloads = Some(r#try!(version.get("downloads")
                .and_then(|c| c.as_i64())
...
fn add_keywords_into_database(conn: &Connection, pkg: &Package, release_id: &i32) -> Result<()> {
    for keyword in &pkg.manifest().metadata().keywords {
        ...
        let keyword_id: i32 = {
            let rows = r#try!(conn.query("SELECT id FROM keywords WHERE slug = $1", &[&slug]));
            if rows.len() > 0 {
                rows.get(0).get(0)
            } else {
                r#try!(conn.query("INSERT INTO keywords (name, slug) VALUES ($1, $2) RETURNING id",
                                &[&keyword, &slug]))
                    .get(0)
                    .get(0)
            }
...
fn add_authors_into_database(conn: &Connection, pkg: &Package, release_id: &i32) -> Result<()> {
    ...
    for author in &pkg.manifest().metadata().authors {
        if let Some(author_captures) = author_capture_re.captures(&author[..]) {
            ...
            let author_id: i32 = {
                let rows = r#try!(conn.query("SELECT id FROM authors WHERE slug = $1", &[&slug]));
                if rows.len() > 0 {
                    rows.get(0).get(0)
                } else {
                    r#try!(conn.query("INSERT INTO authors (name, email, slug) VALUES ($1, $2, $3)
                                     RETURNING id",
                                    &[&author, &email, &slug]))
                        .get(0)
                        .get(0)
...
fn add_owners_into_database(conn: &Connection, pkg: &Package, crate_id: &i32) -> Result<()> {
    ...
    let owners_url = format!("https://crates.io/api/v1/crates/{}/owners",
                             &pkg.manifest().name());
    let client = Client::new();
    let mut res = r#try!(client.get(&owners_url[..])
        .header(ACCEPT, "application/json")
        .send());
    // FIXME: There is probably better way to do this
    //        and so many unwraps...
    ...
    if let Some(owners) = json.as_object()
        ...
        for owner in owners {
            ...
            let owner_id: i32 = {
                let rows = r#try!(conn.query("SELECT id FROM owners WHERE login = $1", &[&login]));
                if rows.len() > 0 {
                    rows.get(0).get(0)
                } else {
                    r#try!(conn.query("INSERT INTO owners (login, avatar, name, email)
                                     VALUES ($1, $2, $3, $4)
                                     RETURNING id",
