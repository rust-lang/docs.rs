...
// REFACTOR: This should be multiple smaller functions
pub(crate) fn add_package_into_database(
    ...
    let release_id: i32 = {
        ...
        if rows.is_empty() {
            // REFACTOR: These are absurdly large
    ...
    // Update versions
    {
        ...
        let mut versions: Json = conn.query(
            "SELECT versions FROM crates WHERE id = $1",
            &[&crate_id]
        )?
        .get(0)
        .get(0);
...
fn get_readme(pkg: &MetadataPackage, source_dir: &Path) -> Result<Option<String>> {
    let readme_path = source_dir.join(pkg.readme.clone().unwrap_or_else(|| "README.md".to_owned()));
