...
pub fn build_doc(name: &str, vers: Option<&str>, target: Option<&str>) -> Result<Package> {
    ...
    let config = r#try!(Config::default());
    let source_id = r#try!(SourceId::crates_io(&config));

    let source_cfg_map = r#try!(SourceConfigMap::new(&config));
    let mut source = r#try!(source_cfg_map.load(source_id, &HashSet::new()));

    // update crates.io-index registry
    r#try!(source.update());

    let dep = r#try!(Dependency::parse_no_deprecated(name, vers, source_id));
    let deps = r#try!(source.query_vec(&dep));
    let pkgid = r#try!(deps.iter().map(|p| p.package_id()).max()
    ...
    let pkg_set = r#try!(PackageSet::new(&[pkgid.clone()], source_map, &config));

    let pkg = r#try!(pkg_set.get_one(pkgid)).clone();

    let current_dir = r#try!(env::current_dir());
    ...
    let source = r#try!(source_cfg_map.load(source_id, &HashSet::new()));
    for (name, dep) in r#try!(resolve_deps(&pkg, &config, source)) {
    ...
    let mut build_config = r#try!(BuildConfig::new(&config,
                                                 None,
    ...
    let ws = r#try!(Workspace::ephemeral(pkg, &config, Some(Filesystem::new(target_dir)), false));
    let exec: Arc<dyn Executor> = Arc::new(DefaultExecutor);
    let source = r#try!(source_cfg_map.load(source_id, &HashSet::new()));
    r#try!(ops::compile_ws(&ws, Some(source), &opts, &exec));

    Ok(r#try!(ws.current()).clone())
}

fn resolve_deps<'cfg>(pkg: &Package, config: &'cfg Config, src: Box<dyn Source + 'cfg>)
    -> CargoResult<Vec<(String, Package)>>
{
    let mut registry = r#try!(PackageRegistry::new(config));
    registry.add_preloaded(src);
    registry.lock_patches();

    let resolver = r#try!(resolver::resolve(
    ...
    let pkg_set = r#try!(registry.get(&dep_ids));
    let deps = r#try!(pkg_set.get_many(dep_ids));
...
pub fn get_package(name: &str, vers: Option<&str>) -> CargoResult<Package> {
    ...
    let source_map = r#try!(SourceConfigMap::new(&config));
    let mut source = r#try!(source_map.load(source_id, &HashSet::new()));
